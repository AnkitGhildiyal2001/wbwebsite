(function ($) {
  ...
  $(document).on('click', '.chat-application .sidebar-profile-toggle', function () {
    $('.chat-profile-sidebar').addClass('show');
    $('.chat-overlay').addClass('show');
  });

  // User Profile sidebar toggle
  $(document).on('click', '.chat-application .user-profile-toggle', function () {
    $('.user-profile-sidebar').addClass('show');
    $('.chat-overlay').addClass('show');
  });

  // Update status by clickin on Radio
  $(document).on('change', '.chat-application .user-status input:radio[name=userStatus]', function () {
  ...
  $(document).on('click', ".chat-application .close-icon", function () {
  ...
  $(document).on('click', ".chat-application .sidebar-close-icon", function () {
    $('.sidebar-content').removeClass('show');
    $('.chat-overlay').removeClass('show');
  });

  // On overlay click
  $(document).on('click', ".chat-application .chat-overlay", function () {
  ...
  $(document).on("click", ".chat-users-list-wrapper li", function () {
    var chatContainer = $(".user-chats");
    chatContainer.animate({ scrollTop: chatContainer[0].scrollHeight }, 400)
  });

  // Favorite star click
  $(document).on("click",".chat-application .favorite i", function (e) {
    $(this).parent('.favorite').toggleClass("warning");
    e.stopPropagation();
  });

  // Main menu toggle should hide app menu
  $(document).on('click','.chat-application .menu-toggle', function (e) {
  ...
  // Chat sidebar toggle
  if ($(window).width() < 992) {
    $(document).on('click', '.chat-application .sidebar-toggle', function () {
  ...
  // Filter
  $(document).on('keyup', ".chat-application #chat-search", function () {
